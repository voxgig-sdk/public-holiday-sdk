...
class PublicHolidayEntity
  ...
  def make
    opts = @_entopts.dup
    PublicHolidayEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PublicHolidayHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PublicHoliday, Hash] the current PublicHoliday data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PublicHolidayHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PublicHoliday fields)
  def match_get
  ...
  #
  # @param reqmatch [PublicHolidayLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PublicHoliday.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PublicHoliday, Hash] the loaded PublicHoliday; raises PublicHolidayError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PublicHolidayHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PublicHolidayListMatch, Hash, nil] match filter (any subset of
  #   PublicHoliday fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PublicHoliday>, Array] the matching PublicHoliday items; raises PublicHolidayError on failure
