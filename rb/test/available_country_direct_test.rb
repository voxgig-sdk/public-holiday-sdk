...
class AvailableCountryDirectTest < Minitest::Test
  def test_direct_list_available_country
    setup = available_country_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-available_country", setup[:live] ? "live" : "unit")
    if _should_skip
...
def available_country_direct_setup(mockres)
