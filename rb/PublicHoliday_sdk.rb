...
class PublicHolidaySDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PublicHolidayHelpers.to_map(feature_opts[fname])
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PublicHolidayError.new(
        "graphql_error", "PublicHolidaySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
