...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = PublicHolidayOmni.make_runner(TEST_JSON_FILE, PublicHolidaySDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    PublicHolidayError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : PublicHolidayOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = PublicHolidayOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
