...
module PublicHolidayUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PublicHolidayResponse.new({})
    ...
    if fetch_err
      ...
      response = PublicHolidayResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PublicHolidayResponse.new(fetched)
    else
