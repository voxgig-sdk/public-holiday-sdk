...
// PublicHoliday SDK retry feature
...
class PublicHolidayRetryFeature extends PublicHolidayBaseFeature
{
    ...
    public function init(PublicHolidayContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PublicHolidayContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PublicHolidayContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PublicHolidayContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
