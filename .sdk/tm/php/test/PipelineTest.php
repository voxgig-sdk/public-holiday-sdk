...
// PublicHoliday SDK pipeline test
//
...
{
    private static function utility(): PublicHolidayUtility
    {
        return new PublicHolidayUtility();
    }
    ...
    private static function ctx(array $over = []): PublicHolidayContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PublicHolidayContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PublicHolidayOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PublicHolidayError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PublicHolidayOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PublicHolidayMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PublicHolidayOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PublicHolidayMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PublicHolidayOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PublicHolidayMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicHolidaySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PublicHolidayMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PublicHolidayResponse([]),
            'result' => new PublicHolidayResult([]),
        ]);
        [, $err] = PublicHolidayMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec([]),
            'response' => null,
            'result' => new PublicHolidayResult([]),
        ]);
        [, $err] = PublicHolidayMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec(['step' => 's']),
            'response' => new PublicHolidayResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PublicHolidayResult([]),
        ]);
        [, $err] = PublicHolidayMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec(['step' => 's']),
            'response' => new PublicHolidayResponse(self::resp(200, ['v' => 1])),
            'result' => new PublicHolidayResult([]),
        ]);
        [, $err] = PublicHolidayMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec(['step' => 's']),
            'response' => new PublicHolidayResponse(self::resp(200, ['v' => 2])),
            'result' => new PublicHolidayResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PublicHolidayMakeResponse::call($ctx);
    ...
    {
        $preset = new PublicHolidayResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec([]),
            'response' => new PublicHolidayResponse([]),
            'result' => new PublicHolidayResult([]),
        ...
        [$got, $err] = PublicHolidayMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PublicHolidayResult([])]);
        [, $err] = PublicHolidayMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PublicHolidaySpec([]), 'result' => null]);
        [, $err] = PublicHolidayMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicHolidaySpec(['step' => 's']),
            'result' => new PublicHolidayResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PublicHolidayOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicHolidayMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicHolidaySpec(['step' => 's']),
            'result' => new PublicHolidayResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PublicHolidayOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicHolidayMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicHolidayResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PublicHolidaySpec([]),
            'result' => new PublicHolidayResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PublicHolidayMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicHolidayMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PublicHolidayError('boom', 'boom');
        $utility->fetcher = function (PublicHolidayContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicHolidaySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PublicHolidayMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PublicHolidayContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PublicHolidayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PublicHolidayResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PublicHolidayMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PublicHolidayContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicHolidaySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PublicHolidayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PublicHolidayResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PublicHolidaySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PublicHolidayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PublicHolidayMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PublicHolidayMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PublicHolidayResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PublicHolidayDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PublicHolidayResult(['ok' => false])]);
        $this->expectException(PublicHolidayError::class);
        PublicHolidayDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PublicHolidayResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PublicHolidayMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PublicHolidayMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PublicHolidayBaseFeature();
        $b = new PublicHolidayBaseFeature();
        PublicHolidayFeatureAdd::call($ctx, $a);
        PublicHolidayFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PublicHolidayBaseFeature
    {
        $f = new PublicHolidayBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PublicHolidayFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PublicHolidayContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PublicHolidaySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicHolidayPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PublicHolidayPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PublicHolidayResponse(['headers' => null]),
            'result' => new PublicHolidayResult([]),
        ]);
        PublicHolidayResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PublicHolidayResultBody::call($ctx);
