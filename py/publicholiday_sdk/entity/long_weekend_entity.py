...
class LongWeekendEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LongWeekendEntity(self._client, opts)
    ...
    def match_get(self) -> LongWeekend:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[LongWeekend]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.LongWeekend().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
