...
from publicholiday_sdk.publicholiday_types import (
    PublicHoliday,
...
class PublicHolidayEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicHolidayEntity(self._client, opts)
    ...
    def data_get(self) -> PublicHoliday:
    ...
    def match_get(self) -> PublicHoliday:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PublicHoliday:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PublicHoliday().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PublicHoliday]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PublicHoliday().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
