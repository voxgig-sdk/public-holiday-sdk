...
from publicholiday_sdk.core.helpers import get_ctx_prop, to_map
...
class PublicHolidayContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = PublicHolidayControl()
        ...
        if isinstance(sp, PublicHolidaySpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PublicHolidayResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PublicHolidayResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return PublicHolidayError(code, msg, self)
