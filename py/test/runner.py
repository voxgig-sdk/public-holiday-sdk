...
class PublicHolidayTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PublicHolidayTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PublicHolidayTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PublicHolidayTestRunner.getenv("PUBLIC_HOLIDAY_TEST_LIVE")
        override = PublicHolidayTestRunner.getenv("PUBLIC_HOLIDAY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PublicHolidayTestRunner.getenv(key)
        ...
        explain = PublicHolidayTestRunner.getenv("PUBLIC_HOLIDAY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PublicHolidayTestRunner._test_control is not None:
            return PublicHolidayTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PublicHolidayTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PublicHolidayTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PublicHolidayTestRunner.load_test_control()
...
def load_env_local():
    PublicHolidayTestRunner.load_env_local()


def env_override(m):
    return PublicHolidayTestRunner.env_override(m)


def entity_data(v):
    return PublicHolidayTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PublicHolidayTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PublicHolidayTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PublicHolidayTestRunner.load_test_control()


def live_client_options():
    return PublicHolidayTestRunner.live_client_options()


def live_delay_ms():
    return PublicHolidayTestRunner.live_delay_ms()
