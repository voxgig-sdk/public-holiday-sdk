...
class TestAvailableCountryDirect:

    def test_should_direct_list_available_country(self):
        setup = _available_country_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-available_country", "live" if setup["live"] else "unit")
        if _skip:
...
def _available_country_direct_setup(mockres):
