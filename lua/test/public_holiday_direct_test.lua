...
describe("PublicHolidayDirect", function()
  it("should direct-list-public_holiday", function()
    local setup = public_holiday_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-public_holiday", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"country_code01", "year01"}) do
  ...
  it("should direct-load-public_holiday", function()
    local setup = public_holiday_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-public_holiday", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function public_holiday_direct_setup(mockres)
