...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:AvailableCountry()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| Field | Type | Description |
| --- | --- | --- |
| `borders` | `table` | Bordering countries |
...
| Field | Type | Description |
| --- | --- | --- |
| `counties` | `table` | If it is not global you found here the Federal states (ISO-3166-2) |
| `countryCode` | `string` | ISO 3166-1 alpha-2 country code |
...
| `types` | `table` | The types of the public holiday. |
