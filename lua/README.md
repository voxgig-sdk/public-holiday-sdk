...
The Lua SDK for the PublicHoliday API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AvailableCountry(nil):list(nil, nil)
...
local result, err = client:PublicHoliday(nil):load(
  { id = "test01" }, nil
)
