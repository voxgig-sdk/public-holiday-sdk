...
local CountryInfoEntity = {}
CountryInfoEntity.__index = CountryInfoEntity


function CountryInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CountryInfoEntity)
  self._name = "country_info"
  self._client = client
...
function CountryInfoEntity:get_name()
  return self._name
end


function CountryInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CountryInfoEntity.new(self._client, opts)
end
...
function CountryInfoEntity:mark_deleted()
  self._deleted = true
end


function CountryInfoEntity:deleted()
  return true == self._deleted
end


function CountryInfoEntity:data_set(args)
  if args ~= nil then
...
function CountryInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CountryInfoEntity:match_set(args)
  if args ~= nil then
...
function CountryInfoEntity:match_get()
...
function CountryInfoEntity:stream(action, args, callopts)
...
function CountryInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
