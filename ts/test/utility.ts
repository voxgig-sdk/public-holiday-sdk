...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PUBLICHOLIDAY_TEST_LIVE ||
    'TRUE' === process.env.PUBLICHOLIDAY_TEST_OVERRIDE
  ) {
  ...
  m.PUBLICHOLIDAY_TEST_EXPLAIN = process.env.PUBLICHOLIDAY_TEST_EXPLAIN || m.PUBLICHOLIDAY_TEST_EXPLAIN
