...
describe('PublicHolidayDirect', async () => {
  ...
  test('direct-load-public_holiday', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-public_holiday', setup.live)) return
  ...
  test('direct-list-public_holiday', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-public_holiday', setup.live)) return
    if (skipIfMissingIds(t, setup, ["country_code01","year01"])) return
    const { client, calls } = setup
