...
describe('CountryInfoDirect', async () => {
  ...
  test('direct-load-country_info', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-country_info', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PUBLIC_HOLIDAY_TEST_COUNTRY_INFO_ENTID']
