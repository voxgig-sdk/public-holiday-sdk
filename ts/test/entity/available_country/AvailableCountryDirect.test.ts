...
describe('AvailableCountryDirect', async () => {
  ...
  test('direct-list-available_country', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-available_country', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PUBLIC_HOLIDAY_TEST_AVAILABLE_COUNTRY_ENTID']
