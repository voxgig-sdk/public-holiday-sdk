...
//     (\`new PublicHolidaySDK(...)\` / \`PublicHolidaySDK.test(...)\` become
//     \`PublicHolidaySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PublicHolidaySDK'
...
const TEST_SEED = {"entity":{"available_country":{"test01":{"id":"test01"}},"country_info":{"test01":{"id":"test01"}},"long_weekend":{"test01":{"id":"test01"}},"public_holiday":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PublicHolidaySDK(...)\` and any
// \`PublicHolidaySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PublicHolidaySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PublicHolidaySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
