...
class PublicHolidaySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PublicHolidayHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PublicHolidayHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PublicHolidayFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PublicHolidayFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PublicHolidayUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PublicHolidayHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PublicHolidayHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PublicHolidayHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PublicHolidayError($op . "_allow",
                "PublicHolidaySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PublicHolidayHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PublicHolidayError("graphql_error",
                "PublicHolidaySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new PublicHolidaySDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
