...
// PublicHoliday SDK feature test
//
...
class FtCtrl extends PublicHolidayControl
{
...
class FtClient
{
    ...
    private PublicHolidayUtility $utility;

    public function __construct(PublicHolidayUtility $utility, array $options)
    {
    ...
    public function get_utility(): PublicHolidayUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PublicHolidayContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PublicHolidayContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PublicHolidayContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PublicHolidayConfig::make_feature($name);
    ...
    private static function buildUrl(PublicHolidaySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PublicHolidayError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PublicHolidayResponse($fetched);
        }
    ...
    private function fail(PublicHolidayContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PublicHolidayContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
