...
import (
	...
	omni "github.com/voxgig-sdk/public-holiday-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PublicHolidayError); is {
				out := map[string]any{
					"name":    "PublicHolidayError",
...
func omniCtx(arg any, client *sdk.PublicHolidaySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PublicHolidaySDK); is {
			client = live
