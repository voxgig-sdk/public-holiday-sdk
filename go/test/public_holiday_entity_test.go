...
func TestPublicHolidayEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.PublicHoliday(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.PublicHoliday(nil).Stream("list", nil, nil) {
...
func public_holidayBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PUBLIC_HOLIDAY_TEST_PUBLIC_HOLIDAY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PUBLIC_HOLIDAY_TEST_PUBLIC_HOLIDAY_ENTID"])
