...
type Args struct {
	Entity string         `json:"entity" jsonschema:"available_country | country_info | long_weekend | public_holiday"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PublicHolidaySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "public-holiday_list",
		Description: "List records from PublicHoliday. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "public-holiday_load",
		Description: "Load a single record from PublicHoliday. " +
...
func runOp(client *sdk.PublicHolidaySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PublicHolidaySDK, name string) (sdk.PublicHolidayEntity, error) {
	switch strings.ToLower(name) {
	case "available_country":
		return client.AvailableCountry(nil), nil
	case "country_info":
		return client.CountryInfo(nil), nil
	case "long_weekend":
		return client.LongWeekend(nil), nil
	case "public_holiday":
		return client.PublicHoliday(nil), nil
