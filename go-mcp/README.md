...
MCP server exposing the PublicHoliday SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `public-holiday_list` | `entity`, optional `query` map | First page of records as JSON |
| `public-holiday_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `public-holiday_list` and
`public-holiday_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"public-holiday_load","arguments":{"entity":"available_country","query":{"id":1}}}}'
```
