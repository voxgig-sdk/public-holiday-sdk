...
[MCP](https://modelcontextprotocol.io) server exposing the PublicHoliday SDK as
two agent tools — `public-holiday_list` and `public-holiday_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/public-holiday-mcp)
make build
...
claude mcp add --scope user public-holiday \
  -- /absolute/path/to/public-holiday-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./public-holiday-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// public-holiday_list: first page of records
{ "entity": "available_country" }
{ "entity": "available_country", "query": { } }

// public-holiday_load: one record by id
{ "entity": "country_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/public-holiday-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user public-holiday \
     -- "$PWD"/dist/*/public-holiday-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `public-holiday_list` and `public-holiday_load` tools now appear
   in new sessions. Ask the agent to *"list available_country using public-holiday"*
   and it calls `public-holiday_list` with `{"entity":"available_country"}`.
...
### Call the `public-holiday_list` tool
...
### Call the `public-holiday_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `public-holiday_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `public-holiday_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./public-holiday-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"public-holiday_load","arguments":{"entity":"country_info","query":{"id":1}}}}'
```
